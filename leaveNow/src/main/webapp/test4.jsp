...
<section>
	...
	<div class="row">
		...
		<!-- 장소 검색 -->
			<div id="drag1" draggable="true" ondragstart="drag(event)" >content1</div>
			<div id="drag2" draggable="true" ondragstart="drag(event)" >content2</div>
			<div id="drag3" draggable="true" ondragstart="drag(event)" >content3</div>
		</div>
...
function drop(ev) {
    ...
    //$target.attr("ondrop","dropDiv(event)");
    
    console.log($target.children("div").html());
    ...
    //$("#"+nodeCopy.id).parent().attr("ondrop","dropDiv(event)");
    //$("#"+nodeCopy.id).attr("ondragstart","dragDiv(event)");
    //var content=document.getElementById(data); 
    //var p_width=ev.target;
    
    //console.log($data.parent().css("background-color","red"));
    //$data.parent().css("postion","");
    
    //$data.parent().css("postion","relative");
    //$data.css("z-index","2").css("background-color","blue").css("postion","absolute");
	...
	for(var i=1; i <= 49;i++){
		...
		if(i%2==0){
			...
			("<tr><th class='col-md-1' >"
					...
					+"<td id='1' class='col-md-3' ondrop='drop(event)' ondragover='return false;' style='text-align: center;padding:0;'><i class='glyphicon glyphicon-plus' style='display:none'></i></td></tr>");
			time+=1;
		}
		else{
			tbody=tbody+("<tr><th class='col-md-1'>"
					+leadingZeros(time, 2)+":"+leadingZeros(min, 2)+"</th>"
					+"<td id='2' class='col-md-3' ondrop='drop(event)' ondragover='return false;' style='text-align: center;padding:0;'><i class='glyphicon glyphicon-plus' style='display:none; '></i></td></tr>");
		}
...
$table1=function(f){
		...
		$("#table-test").append("<div class='col-md-4'><div class='row'><div class='col-md-12'><div class='row'>"
				...
				+"<tr><th colspan='2'>"+f+"일 day</th></tr></thead><tbody>"
				+test()
...
function test1(f){
	var temp=f;
	//temp.next(".glyphicon glyphicon-plus").css("display","none");
