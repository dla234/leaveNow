...
(function (factory) {
  ...
  var costArea = renderer.create('<div class="cost-area" id="costarea"/>');
  ...
  var ui = {
    ...
    button2: function ($node, options) {
        return renderer.create('<button type="button" class="btncostom btncostom-default" tabindex="-1">', function ($node, options) {
          if (options && options.tooltip && self.options.tooltip) {
            $node.attr({
              title: options.tooltip
            }).tooltip({
  ...
  */
  ...
  var Editor = function (context) {
    ...
    this.insertImage = function (src, param) {
      return async.createImage(src, param).then(function ($image) {
        ...
        if (typeof param === 'function') {
          ...
          //--------------------------------------------------------
          $image.css('width', '560px'); // image size fix
          $image.attr('name', 'insertimage'); // create imagename
          //--------------------------------------------------------
        }
    ...
    this.removeMedia = this.wrapCommand(function () {
      ...
      //-------------------------------------------- remove modify
      var filepath = $($target[0]).attr('src');
      var realpath = "D:/java0314/wps/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/summernote/" + filepath;
      
      $.post("/summernote/DelTest", "path=" + realpath); //post() end
          
      /*$.ajax({
		  url: '/summernote/DelTest',
		  data: {"path":realpath},
		  type: "POST",
  ...
  var Buttons = function (context) {
    ...
    this.addToolbarButtons = function () {
      ...
      /*context.memo('button.cost', function () {
          return ui.button({
            contents: "$",
            tooltip: lang.image.cost,
            click: CostTest
          }).render();
        });*/
  ...
  var CostTest = function(){
	  $("#costarea").append("<div id='costtable' style='border-top:1px solid #999;'>" +
			  					"<select name='toggle' id='toggle' style='align:center; height:30px; margin:2px 4px 2px 4px;'>" +
			  						"<option value='0'>입장료</option>" +
			  					...
			  					"</select>" +
			  					"<input type='text' name='paylist' id='note-list-input' maxlength='100' style='width:370px; margin-right:4px;' placeholder='예)기념품 사기 or 음식 준비'>" +
			  					"<input type='text' name='mypay' id='note-cost-input' onchange='getNumber(this);' onkeyup='getNumber(this);' maxlength='100' style='width:115px; margin-right:4px; text-align:right;'>" +
			  					"<input type='button' class='note-btn btn btn-default btn-sm' value='X' onclick='Remove_costtable()'>" +
	  						"</div>");
  }
